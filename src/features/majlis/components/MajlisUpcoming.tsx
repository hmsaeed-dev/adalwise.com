...
export function MajlisUpcoming({ doc }: MajlisUpcomingProps) {
  ...
  return (
    <section aria-labelledby="upcoming-majlis-heading" className="border-b border-brand-primary/15 pb-20 mb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-4 flex flex-col justify-between">
          <div>
            ...
            <p className="mt-4 font-sans text-xs uppercase tracking-widest text-brand-primary/60">
              Convocated by {session.host?.name || "Dr. Hafiz Haseeb"}
            </p>
          </div>

          <div className="mt-8 space-y-2 font-sans text-sm text-brand-primary/75">
            <div className="flex items-center gap-2.5">
              <Calendar className="w-4 h-4 text-brand-primary/70 shrink-0" aria-hidden="true" />
              <span>{formattedDate}</span>
            </div>
            <div className="flex items-center gap-2.5">
              <MapPin className="w-4 h-4 text-brand-primary/70 shrink-0" aria-hidden="true" />
              <span>
        ...
        <div className="lg:col-span-8 flex flex-col justify-between space-y-8">
          <div>
            ...
            {(session.thesis || session.description) && (
              <p className="font-serif italic text-lg sm:text-xl text-brand-primary/85 leading-relaxed max-w-3xl">
          ...
          {inquiries.length > 0 && (
            <div className="bg-brand-parchment/60 border-l-2 border-brand-gold p-6 sm:p-7 space-y-4 rounded-r-lg">
              <h3 className="font-sans text-xs uppercase tracking-[0.2em] font-semibold text-brand-primary/70">
                Primary Inquiries for Deliberation
              </h3>
