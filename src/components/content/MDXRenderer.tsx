    ...
    <div className="flex flex-col gap-space-md text-[16px] sm:text-[17px] leading-relaxed text-on-surface">
      {paragraphs.map((block, idx) => {
        ...
        if (trimmed.startsWith("- ") || trimmed.startsWith("* ")) {
          ...
          return (
            <ul key={idx} className="flex flex-col gap-2 my-space-xs pl-space-md">
              {items.map((item, itemIdx) => (
                <li key={itemIdx} className="flex items-start gap-space-xs">
                  <span className="text-tertiary-container font-bold mt-1 select-none">♦</span>
        ...
        if (/^\d+\.\s/.test(trimmed)) {
          ...
          return (
            <ol key={idx} className="flex flex-col gap-2 my-space-xs pl-space-md list-decimal">
              {items.map((item, itemIdx) => (
